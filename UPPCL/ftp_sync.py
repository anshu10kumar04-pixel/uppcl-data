...
def ask_proceed(msg):
    try:
        ...
        print(f"  ALREADY SYNCED: {msg} — skipped")
        return False
...
def shard_master(rows, div_key):
    ...
    # Detect indices from first row keys
    keys = list(rows[0].keys())
    hdrs = [k.strip().upper() for k in keys]
    
    i_acct = find_col(hdrs, ['ACCT_ID', 'ACCOUNT_ID'])
    i_mob  = find_col(hdrs, ['MOBILE_NO', 'MOBILE'])
    i_name = find_col(hdrs, ['NAME', 'CONSUMER_NAME'])
    i_addr = find_col(hdrs, ['ADDRESS', 'ADDRES'])
    i_cat  = find_col(hdrs, ['CATEGORY', 'TARIFF_TYPE', 'SUPPLY_TYPE'])
    i_load = find_col(hdrs, ['LOAD', 'SANCTION_LOAD'])
    i_lat  = find_col(hdrs, ['LAT', 'LATITUDE'])
    i_lon  = find_col(hdrs, ['LON', 'LONGITUDE'])
    i_sub  = find_col(hdrs, ['SUBSTATION', 'SUB_STATION'])
    i_feed = find_col(hdrs, ['FEEDER'])
    i_date = find_col(hdrs, ['PAY_DATE', 'LP DATE', 'LAST_PAY_DATE'])
    i_amt  = find_col(hdrs, ['PAY_AMT', 'LP AMT', 'LAST_PAY_AMT'])
    i_out  = find_col(hdrs, ['TOTAL_OUTSTANDING', 'TOTAL OUTSTANDING', 'AMOUNT_PAYABLE'])
    ...
    for r in rows:
        ...
        sub = str(vals[i_sub] if i_sub >= 0 else '').strip()
    ...
    for div_code, div_key in divisions.items():
        ...
        if 'master' in sync_types:
            try:
                ...
                if force and prev_status.get('master_period') == month_dir:
                    pass
                elif prev_status.get('master_period') == month_dir:
                    if not ask_proceed(f"Master DIV{div_code} {month_dir} already synced."):
                        raise StopIteration
        ...
        if 'billed' in sync_types:
            try:
                ...
                if force and prev_status.get('billed_date') == ddmmyyyy:
                    pass
                elif prev_status.get('billed_date') == ddmmyyyy:
                    if not ask_proceed(f"Billed DIV{div_code} {ddmmyyyy} already synced."):
                        raise StopIteration
                raw = ftp_download_gz(ftp, path)
        ...
        if 'unbilled' in sync_types:
            try:
                ...
                if force and prev_status.get('unbilled_date') == ddmmyyyy:
                    pass
                elif prev_status.get('unbilled_date') == ddmmyyyy:
                    if not ask_proceed(f"Unbilled DIV{div_code} {ddmmyyyy} already synced."):
                        raise StopIteration
                raw = ftp_download_gz(ftp, path)
